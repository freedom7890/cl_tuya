...
extern uint8_t PERCENT_STATE,PERCENT_CONTROL,AUTO_POWER,BATTERY_PERCENTAGE;
extern uint8_t Last_STATE,CONTROL,auto_power_flag;
extern uint16_t now_control,max,now;
...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_enum_update(DPID_CONTROL,CONTROL); //枚举型数据上报;
    mcu_dp_value_update(DPID_PERCENT_CONTROL,PERCENT_CONTROL); //VALUE型数据上报;    0
    mcu_dp_value_update(DPID_PERCENT_STATE,PERCENT_STATE); //VALUE型数据上报;
//    mcu_dp_enum_update(DPID_MODE,当前工作模式); //枚举型数据上报;
//    mcu_dp_enum_update(DPID_CONTROL_BACK,当前电机反向); //枚举型数据上报;
    mcu_dp_bool_update(DPID_AUTO_POWER,AUTO_POWER); //BOOL型数据上报; 
...
//   mcu_dp_enum_update(DPID_SITUATION_SET,当前百分比含义); //枚举型数据上报;
//    mcu_dp_fault_update(DPID_FAULT,当前故障告警); //故障型数据上报;
   mcu_dp_value_update(DPID_BATTERY_PERCENTAGE,BATTERY_PERCENTAGE); //VALUE型数据上报;
//   mcu_dp_enum_update(DPID_BORDER,当前设置限位); //枚举型数据上报;
//    mcu_dp_value_update(DPID_POSITION_BEST,当前最佳位置); //VALUE型数据上报;
//    mcu_dp_enum_update(DPID_CLICK_CONTROL,当前点动控制); //枚举型数据上报;
//    mcu_dp_value_update(DPID_ANGLE_HORIZONTAL,当前角度控制); //VALUE型数据上报;
...
static unsigned char dp_download_percent_control_handle(const unsigned char value[], unsigned short length)
{
    ...
    //VALUE类型数据处理
    now_control=percent_control*max/100;
	if(now_control>now)
	{
		close();
	}
	else if(now_control<now)
	{
    open();
	}
    //处理完DP数据后应有反馈
...
//}
/*****************************************************************************
函数名称 : dp_download_auto_power_handle
功能描述 : 针对DPID_AUTO_POWER的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_auto_power_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char auto_power;
    
    auto_power = mcu_get_dp_download_bool(value,length);
    if(auto_power == 0) {
        //开关关
			AUTO_POWER=0;
    }else {
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_AUTO_POWER,auto_power);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
//}
/*****************************************************************************
函数名称 : dp_download_click_control_handle
功能描述 : 针对DPID_CLICK_CONTROL的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
//static unsigned char dp_download_click_control_handle(const unsigned char value[], unsigned short length)
//{
//    //示例:当前DP类型为ENUM
//    unsigned char ret;
//    unsigned char click_control;
//    
//    click_control = mcu_get_dp_download_enum(value,length);
//    switch(click_control) {
//        case 0:
//        break;
//        
//        case 1:
...
//    
//    //处理完DP数据后应有反馈
//    ret = mcu_dp_enum_update(DPID_CLICK_CONTROL, click_control);
//    if(ret == SUCCESS)
//        return SUCCESS;
//    else
//        return ERROR;
//}
/*****************************************************************************
函数名称 : dp_download_angle_horizontal_handle
功能描述 : 针对DPID_ANGLE_HORIZONTAL的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
//static unsigned char dp_download_angle_horizontal_handle(const unsigned char value[], unsigned short length)
//{
//    //示例:当前DP类型为VALUE
//    unsigned char ret;
//    unsigned long angle_horizontal;
//    
//    angle_horizontal = mcu_get_dp_download_value(value,length);
//    /*
//    //VALUE类型数据处理
//    
//    */
//    
//    //处理完DP数据后应有反馈
//    ret = mcu_dp_value_update(DPID_ANGLE_HORIZONTAL,angle_horizontal);
//    if(ret == SUCCESS)
//        return SUCCESS;
//    else
//        return ERROR;
...
//        break;
        case DPID_AUTO_POWER:
            //自启动开关处理函数
            ret = dp_download_auto_power_handle(value,length);
        break;
...
//        break;
//        case DPID_CLICK_CONTROL:
//            //点动控制处理函数
//            ret = dp_download_click_control_handle(value,length);
//        break;
//        case DPID_ANGLE_HORIZONTAL:
//            //角度控制处理函数
//            ret = dp_download_angle_horizontal_handle(value,length);
//        break;
